...
class TestMaxInteger(unittest.TestCase):
    ...
    def test_function_docstring(self):
        """It would test for funstion docstring"""
        f = max_integer.__doc__
        self.assertTrue(len(f) > 1)

    def test_empty_list(self):
        """It would test for empty list []"""
        e = []
        self.assertIsNone(max_integer(e))

    def test_no_args(self):
        """It would test for no arguments passed to func"""
        self.assertIsNone(max_integer())

    def test_one_element(self):
        """It would test for only one number in the list"""
        o = [1]
        self.assertEqual(max_integer(o), 1)

    def test_positive_end(self):
        """Must Test for all positive with max at end"""
        e = [2, 10, 8, 36, 14, 50]
        self.assertEqual(max_integer(e), 50)

    def test_positive_middle(self):
        """Must Test for all positive with max in middle"""
        m = [2, 10, 8, 360, 14, 50]
